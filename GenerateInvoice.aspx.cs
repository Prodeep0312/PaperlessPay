...
namespace Project
{
    public partial class GenerateInvoice : System.Web.UI.Page
    {
     ...
     "THIRTEEN", "FOURTEEN","FIFTEEN", "SIXTEEN", "SEVENTEEN", "EIGHTEEN", "NINETEEN" };
        ...
        {
            string pdfName = "File_" + DateTime.Now.ToString("ddMMyyyyhhmm") + ".pdf";
            string username = Request.QueryString["username"];
            ...
            string course = DropDownList2.SelectedItem.Text; ;
            decimal fees = decimal.Parse(TextBox4.Text);

            DatabaseHelper dbHelper = new DatabaseHelper();
            dbHelper.InsertInvoice(username, studentName, address, paymentType, date, course, fees, pdfName);

            ClientScript.RegisterStartupScript(this.GetType(), "Invoice Saved in DB!", "alert('Invoice Saved in DB!');", true);
        ...
        protected void GeneratePDFButton_Click(object sender, EventArgs e)
        {
            ...
            sb.Append("<td align='center'>" + DropDownList2.SelectedItem.Text + "</td>");
            ...
            string barcodeImagePath = Server.MapPath("~/Img/barcode" + counter + ".jpeg");
            while (File.Exists(barcodeImagePath))
            {
                counter++;
                barcodeImagePath = Server.MapPath("~/Img/barcode" + counter + ".jpeg");
            }
            ...
            string filename = "File_" + DateTime.Now.ToString("ddMMyyyyhhmm") + ".pdf";
